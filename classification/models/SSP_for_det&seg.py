...
class Attention(nn.Module):
    def __init__(self, dim, mask, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,
        ...
        if sr_ratio > 1:
            if mask:
                ...
                self.wi = nn.Conv2d(dim, dim, 1, bias=qkv_bias)
        else:
            self.q = nn.Linear(dim, dim, bias=qkv_bias)
   ...
   def forward(self, x, H, W, mask):
        ...
        if self.sr_ratio > 1:
            if mask is None:
                wv1 = self.w1(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
                x_ = x.permute(0, 2, 1).reshape(B, C, H, W)
                x_1 = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1)
                x_1 = self.act(self.norm(x_1))
                wv2 = self.w2(x_1).reshape(B, -1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
                I = self.wi(x_1.permute(0, 2, 1).reshape(B, C, H // self.sr_ratio, W // self.sr_ratio)).reshape(B, -1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)

                a = (wv1 @ wv2.transpose(-2, -1)) * self.scale
                A = a.softmax(dim=-1)
                A = self.attn_drop(A)
                mask_a = torch.mean(A.detach().mean(1), dim=1)  # B Nk  #max ?  mean ?
                mask_a = F.interpolate(mask_a.view(B, 1, H // self.sr_ratio, W // self.sr_ratio),
                                                  (H, W), mode='nearest')    # b 1 h w
                mask_a = mask_a.view(B, 1, H * W).permute(0, 2, 1) # b n 1
                
                I1 = (A @ I).transpose(1, 2).reshape(B, N, C)
                x = I1 * mask_a
                x = self.proj(x + lepe)
                x = self.proj_drop(x)
                ...
                if self.sr_ratio == 8:
                    ...
                    token1, token2, token3 = token1 // 4, token2 // 2, token3 // 4  # 4,
                elif self.sr_ratio == 4:
                ...
                if self.sr_ratio == 8:
                    p1 = torch.gather(x, 1,
                                      mask_sort_index1[:, :token1 * 14 * 14].unsqueeze(-1).repeat(1, 1, C))  # B, N//4, C
                    p2 = torch.gather(x, 1,
                                      mask_sort_index1[:, token1 * 14 * 14:token2 * 56 + token1 * 14 * 14].unsqueeze(-1).repeat(1, 1, C))
                    p3 = torch.gather(x, 1, mask_sort_index1[:, token2 * 56 + token1 * 14 * 14:token3 * 28 + token2 * 56 + token1 * 14 * 14].unsqueeze(-1).repeat(1, 1, C))
                    ...
                    p1 = torch.gather(x, 1,
                                      mask_sort_index1[:, :token1 * 49].unsqueeze(-1).repeat(1, 1, C))  # B, N//4, C
                    p2 = torch.gather(x, 1,
                                      mask_sort_index1[:, token1 * 49:token2 * 14 + token1 * 49].unsqueeze(-1).repeat(1, 1, C))
                    p3 = torch.gather(x, 1,
                                      mask_sort_index1[:, token2 * 14 + token1 * 49:token3 * 7 + token2 * 14 + token1 * 49].unsqueeze(-1).repeat(1, 1, C))
                    ...
                    p3_ = torch.gather(x, 1,
                                       mask_sort_index2[:, -(token1 + token2 + token3):].unsqueeze(-1).repeat(1, 1, C))

                    seq2 = p3_  # B N C

                elif self.sr_ratio == 2:
                    p1 = torch.gather(x, 1,
                                      mask_sort_index1[:, :token1 * 2].unsqueeze(-1).repeat(1, 1, C))  # B, N//4, C
                    p2 = torch.gather(x, 1, mask_sort_index1[:, token1 * 2:token1 * 2 + token2].unsqueeze(-1).repeat(1, 1, C))
                    seq1 = torch.cat([self.f1(p1.permute(0, 2, 1).reshape(B, C, token1, -1)).squeeze(-1),
                                      self.f2(p2.permute(0, 2, 1).reshape(B, C, token2, -1)).squeeze(-1)],
                                     dim=-1).permute(0, 2, 1)  # B N C
