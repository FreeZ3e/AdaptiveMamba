...
# ssp
try:
    from .SSP import SSP_block
except:
    from SSP import SSP_block


class ssp(nn.Module):
    def __init__(self, embed_dim, sr_ratio=8):
        super().__init__()

        self.SSP_block = SSP_block(embed_dim=embed_dim, sr_ratio=sr_ratio)
...
class c_scan(nn.Module, c_scan_layer):
    def __init__(
            self,
            # basic dims ===========
            d_model=96,
...
class AdaptiveMamba(nn.Module):
    def __init__(
        ...
        self.sr_ratios = [8, 4, 2, 1]  # sr_ratio for self-guided attn patch size
        ...
        for i_layer in range(self.num_layers):
            ...
            self.layers.append(self._make_layer(
                layer=i_layer,  # insert self-guided attn in stage 1,2,3
                sr_ratio=self.sr_ratios[i_layer],  # sr_ratio for self-guided attn
    ...
    def _make_layer(
        ...
        if layer < 3:
            print("ssp block")
            blocks += nn.Sequential(ssp(embed_dim=dim, sr_ratio=sr_ratio))

        for d in range(depth):
            blocks += nn.Sequential(c_scan_block(
                hidden_dim=dim,
        ...
        blocks += nn.Sequential(downsample)
...
class Backbone_AdaptiveMamba(AdaptiveMamba):
